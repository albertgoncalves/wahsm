(module
      ...
      (i32.rem_u
        (i32.add
          (i32.load offset=0 align=4
            (i32.shl (i32.rem_u (local.get $x) (global.get $N)) (i32.const 2))
          )
          (i32.load offset=0 align=4
            (i32.shl (i32.rem_u (local.get $y) (global.get $N)) (i32.const 2))
          )
        )
        (global.get $N)
      )
  ...
  (func (export "main")
    ...
    (local.set $min (f32.const 1.0))
    (block $break
      (local.set $y (i32.const 0))
      (loop $y_continue
        (local.set $y_n (i32.mul (local.get $y) (global.get $N)))
        ...
        (loop $x_continue
          ...
          (br_if $x_continue (i32.lt_u (local.get $x) (global.get $N)))
        )
        (local.set $y (i32.add (local.get $y) (i32.const 1)))
        (br_if $y_continue (i32.lt_u (local.get $y) (global.get $N)))
      )
      (local.set $delta (f32.sub (local.get $max) (local.get $min)))
      (local.set $i (i32.const 0))
      (loop $i_continue
        (local.set $_i (i32.shl (local.get $i) (i32.const 2)))
        (local.set $value
          (f32.mul
            (f32.div
              (f32.sub
                (f32.load offset=1536 align=4 (local.get $_i))
                (local.get $min)
              )
              (local.get $delta)
            )
        ...
        (local.set $value_i (i32.trunc_f32_u (local.get $value)))
        (i32.store8 offset=1536 align=1 (local.get $_i) (local.get $value_i))
        (i32.store8 offset=1537 align=1 (local.get $_i) (local.get $value_i))
        (i32.store8 offset=1538 align=1 (local.get $_i) (local.get $value_i))
        (i32.store8 offset=1539 align=1 (local.get $_i) (i32.const 255))
        (local.set $i (i32.add (local.get $i) (i32.const 1)))
        (br_if $i_continue (i32.lt_u (local.get $i) (global.get $NN)))
      )
    )
  )
)
